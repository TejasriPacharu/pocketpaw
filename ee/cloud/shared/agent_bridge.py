...
When a message is sent in a group with agents, this module:
1. Checks each agent's respond_mode (silent, auto, mention_only, smart)
2. Triggers agents that should respond
3. Streams agent responses back to the group via WebSocket
"""
...
async def _run_agent_response(
    ...
    if ripple_spec:
        try:
            ...
            pocket_name = ripple_spec.get("lifecycle", {}).get("name") or ripple_spec.get("name") or f"{instance.agent_name} — {user_message[:40]}"
            pocket = Pocket(
                workspace=workspace_id,
                name=pocket_name,
                description=f"Generated by {instance.agent_name}",
                type="ai-generated",
                owner=group_members[0] if group_members else "",
                rippleSpec=ripple_spec,
                visibility="workspace",
            )
            await pocket.insert()
            pocket_id = str(pocket.id)
            logger.info("Auto-created pocket %s from agent %s ripple spec", pocket_id, instance.agent_name)
        except Exception:
